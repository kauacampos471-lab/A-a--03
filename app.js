...
function refreshCurrentScreen(tab) {
  ...
  if (tab === 'produtos') { Criacao.render(); Config.renderProdutos(); }
...
const Estoque = (() => {
  ...
  function progressPct(item) {
    const ideal = item.estoqueIdeal || 1;
    return Math.max(0, Math.min(100, (item.quantidade / ideal) * 100));
  }
  ...
  function renderList() {
    ...
    list.forEach(item => {
      ...
      const row = document.createElement('div');
      row.className = 'stock-card ' + cls;
      row.innerHTML = `
        <div class="stock-card-top">
          <div>
            <div class="li-main">${emojis}${item.nome}${item.isAdicional ? ' · 🥄 adicional' : ''}</div>
            <div class="li-sub">${item.quantidade}${item.unidade} · mínimo ${item.estoqueIdeal}${item.unidade}</div>
          </div>
          <div class="stock-stepper">
            <button class="minus" data-minus="${item.id}">−</button>
            <button class="plus" data-plus="${item.id}">+</button>
          </div>
        </div>
        <div class="stock-progress-track"><div class="stock-progress-fill" style="width:${progressPct(item)}%"></div></div>
...
const Financeiro = (() => {
  ...
  function renderChart() {
    ...
    const bruta = dias.map(d => vendas.filter(v => new Date(v.dataHora).toDateString() === d.date.toDateString()).reduce((s, v) => s + v.total, 0));
    const liquido = dias.map(d => vendas.filter(v => new Date(v.dataHora).toDateString() === d.date.toDateString()).reduce((s, v) => s + v.lucro, 0));
    drawBarChart(canvas, dias.map(d => d.label), [{ data: bruta, color: '#5b8ef2' }, { data: liquido, color: '#b98cf2' }]);
  }
  ...
  function renderPagamentos() {
    ...
    const porForma = {};
    vendas.forEach(v => { porForma[v.formaPagamento] = (porForma[v.formaPagamento] || 0) + v.total; });
  ...
  const MEDALS = ['🥇', '🥈', '🥉'];
  function rankLabel(idx) { return MEDALS[idx] || `#${idx + 1}`; }
  ...
  function renderTop() {
    ...
    ranked.forEach((r, idx) => {
      ...
      const row = document.createElement('div');
      row.className = 'list-item';
      row.innerHTML = `<span>${rankLabel(idx)} ${r.nome} (${r.qtd}x)</span><span>${brl(r.fat)} · ${pct}%</span>`;
      container.appendChild(row);
    });
    ...
    rankedLucro.forEach((r, idx) => {
      const row = document.createElement('div');
      row.className = 'list-item';
      row.innerHTML = `<span>${rankLabel(idx)} ${r.nome}</span><span>Lucro ${brl(r.lucro)}</span>`;
    ...
    vendas.forEach(v => {
      ...
      row.innerHTML = `
        <div class="hist-row-head" data-toggle="${v.id}">
          <div>
            <div class="li-main">${fmtDateTime(v.dataHora)}</div>
